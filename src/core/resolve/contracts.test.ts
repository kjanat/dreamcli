...
describe('resolver contracts — explicit facts', () => {
...
describe('resolver contracts — flag precedence matrix', () => {
...
describe('resolver contracts — arg precedence matrix', () => {
	const schema = makeSchema({
		args: [
			{
				name: 'target',
...
describe('resolver contracts — hard errors and aggregation', () => {
	...
	it('keeps stdin coercion errors authoritative for args instead of falling through', async () => {
		...
		expect(error.code).toBe('TYPE_MISMATCH');
		expect(error.message).toBe("Invalid number value '<redacted>' from stdin for argument <count>");
		expect(error.details).toEqual({
			arg: 'count',
			source: 'stdin',
			expected: 'number',
