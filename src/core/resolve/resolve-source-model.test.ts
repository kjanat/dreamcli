...
describe('arg config and prompt sources', () => {
	...
	it('says an arg kind with no compatible prompt is not promptable', async () => {
		const schema = command('deploy')
			.arg(
				'vars',
				new ArgBuilder(
					createArgSchema('keyValue', { prompt: { kind: 'input', message: 'Vars?' } }),
				),
			)
			.action(() => {}).schema;

		await expect(
			resolve(schema, parse(schema, []), { prompter: createTestPrompter(['A=1']) }),
		).rejects.toMatchObject({
			code: 'CONSTRAINT_VIOLATED',
			message:
				"Prompt kind 'input' is not compatible with keyValue argument <vars>. keyValue arguments are not promptable",
			suggest: 'Remove the prompt config for <vars>',
			details: { arg: 'vars', argKind: 'keyValue', promptKind: 'input', allowed: [] },
		});
	});
