...
describe('an input that collects several values takes the same prompt on both surfaces', () => {
	...
	it('leaves a key-value positional unpromptable, as its flag twin is', async () => {
		const prompt: PromptConfig = { kind: 'input', message: 'Pick' };
		const flagSchema = createCommandSchema({
			name: 'subject',
			flags: { x: createFlagSchema('keyValue', { prompt }) },
		});
		const argSchema = createCommandSchema({
			name: 'subject',
			args: [{ name: 'x', schema: createArgSchema('keyValue', { prompt }) }],
		});

		const forFlag = await promptVerdict(flagSchema, 'a=1');
		const forArg = await promptVerdict(argSchema, 'a=1');

		expect(forFlag.reason).toBe(
			"Prompt kind 'input' is not compatible with keyValue <subject>. keyValue flags are not promptable",
		);
		expect(forArg.reason).toBe(
			"Prompt kind 'input' is not compatible with keyValue <subject>. keyValue arguments are not promptable",
		);
	});
});
