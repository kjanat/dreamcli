...
import type { CompletionOptions, Shell } from './index.ts';
import {
	detectShell,
...
// === Script headers — program version

describe('script headers, program version', () => {
	const headers: readonly (readonly [Shell, string])[] = [
		['bash', '# Bash completion for '],
		['zsh', '# Zsh completion for '],
		['fish', '# Fish completion for '],
		['powershell', '# PowerShell completion for '],
	];

	for (const [shell, prefix] of headers) {
		it(`${shell} header names the program and its version`, () => {
			const script = generateCompletion(minimalSchema({ version: '2.3.4' }), shell);

			expect(script).toContain(`${prefix}testcli v2.3.4\n`);
			expect(script).toContain('# Generated by @kjanat/dreamcli');
		});

		it(`${shell} header omits the version when the program has none`, () => {
			const script = generateCompletion(minimalSchema({ version: undefined }), shell);

			expect(script).toContain(`${prefix}testcli\n`);
		});
	}
});
