...
describe('CLIBuilder.run()', () => {
	...
	describe('config errors', () => {
		it('renders CONFIG_NOT_FOUND error when explicit path missing', async () => {
			...
			const { stderr, exitCode } = await runWithAdapter(app, [
				'--config',
				'/missing.json',
				'deploy',
			]);
	...
	describe('completions skip config', () => {
...
// === CLIBuilder.configLoader()

describe('CLIBuilder.configLoader()', () => {
	describe('builder method', () => {
		it('returns a new CLIBuilder (immutability)', () => {
