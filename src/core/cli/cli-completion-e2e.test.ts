...
describe('.completions()', () => {
	...
	describe('schema coverage', () => {
		describe('bash', () => {
			it('reflects the full schema', async () => {
			...
			it('supports middleware-enhanced commands', async () => {
	...
	describe('error paths', () => {
		...
		it('renders JSON errors with --json', async () => {
			const app = cli('myapp').command(deployCommand()).completions();
	...
	describe('metadata', () => {
		describe('bash', () => {
			it('uses the CLI name from the builder', async () => {
		...
		describe('zsh', () => {
			it('uses the CLI name in the compdef directive', async () => {
		...
		describe('command-free CLIs', () => {
			describe('bash', () => {
				it('keeps only global flags', async () => {
				...
				it('omits --version when unset', async () => {
	...
	describe('nested commands', () => {
		describe('bash', () => {
			...
			it('includes propagated flags', async () => {
			...
			it('lets group commands complete subcommands and own flags', async () => {
				const app = cli('myapp').command(dbCommand()).completions();
				const result = await app.execute(['completions', 'bash']);
				const script = result.stdout.join('');
		...
		describe('zsh', () => {
			it('generates helper functions', async () => {
				const app = cli('myapp').command(dbCommand()).command(deployCommand()).completions();
				const result = await app.execute(['completions', 'zsh']);
				expect(result.exitCode).toBe(0);
				const script = result.stdout.join('');
			...
			it('lists subcommands via _describe', async () => {
				const app = cli('myapp').command(dbCommand()).completions();
				const result = await app.execute(['completions', 'zsh']);
				const script = result.stdout.join('');
...
describe('CLIBuilder.run()', () => {
	describe('auto-adapter wiring', () => {
