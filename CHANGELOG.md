...
- **Definition types and normalization factories for flags, args, commands, and
  ...
  `createCLISchema()` join the existing `createArgSchema()`. Each takes a plain
  definition object, or a kind plus that kind's fields, and returns the
  normalized schema; feeding a built schema back in produces a deep-equal
...
- **Purpose-built positional argument kinds and string constraints**
  ...
  so byte-for-byte equality with the flag message holds for argv-sourced values.
