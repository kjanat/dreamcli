...
- `dreamcli.schema.json` is tracked. Changes to source JSDoc can update `src/core/json-schema/meta-descriptions.generated.ts` and then the root schema. Run `bun run meta-descriptions` and `bun run schema:emit`, and commit both generated changes when needed. The emitter applies the configured dprint schema ordering; docs CI rejects any tracked or untracked changes left by the build.
