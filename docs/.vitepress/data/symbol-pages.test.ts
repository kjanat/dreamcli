...
describe('symbol page generation', () => {
  it('renders stable per-symbol routes from the normalized TypeDoc model', {
    ...
    const middlewareInterfacePage = pages.find(
      (page) => page.id === '@kjanat/dreamcli:Middleware',
    );
    ...
    expect(cliPage?.content).toContain(
      '- [Basic single-command CLI.](/examples/basic)',
    );
    ...
    expect(middlewareInterfacePage).toMatchObject({
      ...
      filePath: expect.stringContaining(
        '/docs/reference/symbols/main/middleware-type.md',
      ),
    });
  });
  ...
  it('renders parameter tables with escaped markdown pipes but plain type unions', {
    ...
    const runCommandPage = pages.find(
      (page) => page.id === '@kjanat/dreamcli/testkit:runCommand',
    );

    expect(runCommandPage?.content).toContain(
  ...
  it('keeps API index links aligned with rendered symbol routes', () => {
    expect(toSymbolPageRoute('@kjanat/dreamcli', 'cli')).toBe(
      '/reference/symbols/main/cli',
    );
    expect(
    ...
    expect(
      toSymbolPageRoute('@kjanat/dreamcli/runtime', 'RuntimeAdapter'),
    ).toBe('/reference/symbols/runtime/RuntimeAdapter');
