...
import { gitRef, tsconfigPath } from './paths.ts';
...
export async function collectTypeDocModel(
  ...
  const moduleMap = new Map<string, JSONOutput.DeclarationReflection>(
    (rawProject.children ?? []).map((entrypoint) => [
      entrypoint.name,
      entrypoint,
    ]),
  );
  ...
  const entrypoints = publicApi.map((entrypoint) => {
    ...
    for (const symbol of flattenEntrypointSymbols(entrypoint)) {
      const reflection = moduleChildren.find(
        (child) => child.name === symbol.name,
      );
...
function flattenEntrypointSymbols(
...
function normalizeReflection(reflection: TypeDocNode): NormalizedApiNode {
  const children =
    ('children' in reflection ? reflection.children : undefined)?.map(
      normalizeReflection,
    ) ?? [];
  ...
  const signatures =
    ('signatures' in reflection ? reflection.signatures : undefined)?.map(
      normalizeReflection,
    ) ?? [];
  const parameters =
    ('parameters' in reflection ? reflection.parameters : undefined)?.map(
      normalizeReflection,
    ) ?? [];
  ...
  return {
    ...
    defaultValue:
      'defaultValue' in reflection && reflection.defaultValue !== undefined
        ...
        : null,
    type:
      'type' in reflection && reflection.type !== undefined
        ? normalizeType(reflection.type)
        : null,
    signatures,
    parameters,
    typeParameters,
    ...
    groups: normalizeGroups(
      'groups' in reflection ? reflection.groups : undefined,
      children,
    ),
  };
}
...
function displayPartsToText(
  parts: readonly JSONOutput.CommentDisplayPart[],
): string {
  return parts.map((part) => part.text).join('');
}

function getSources(
  reflection: TypeDocNode,
): readonly JSONOutput.SourceReference[] {
...
function normalizeSourcePath(
  sources: readonly JSONOutput.SourceReference[],
): string | null {
  ...
  switch (type.type) {
    ...
    case 'inferred':
      return {
        ...
        name: type.name,
        constraint:
          type.constraint === undefined ? null : normalizeType(type.constraint),
      };
    case 'intersection':
      ...
      return {
        ...
        parameterType:
          type.parameterType === undefined
            ? null
            : normalizeType(type.parameterType),
        templateType:
          type.templateType === undefined
            ? null
            : normalizeType(type.templateType),
        readonlyModifier: type.readonlyModifier ?? null,
        optionalModifier: type.optionalModifier ?? null,
        nameType:
          type.nameType === undefined ? null : normalizeType(type.nameType),
      };
    ...
    case 'predicate':
      return {
        ...
        targetType:
          type.targetType === undefined ? null : normalizeType(type.targetType),
      };
    case 'query':
    ...
    case 'reflection':
      return {
        kind: 'reflection',
        declaration: normalizeReflection(type.declaration),
      };
    case 'rest':
    ...
    case 'templateLiteral':
      return {
        kind: 'templateLiteral',
        head: type.head,
        tail: type.tail.map(([tailType, text]) => ({
          type: normalizeType(tailType),
          text,
        })),
      };
    case 'tuple':
      return {
        kind: 'tuple',
        elements: (type.elements ?? []).map(normalizeType),
      };
    case 'typeOperator':
      return {
        kind: 'typeOperator',
