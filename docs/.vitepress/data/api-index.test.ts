...
describe('api-index', () => {
  it('collects every public entrypoint with grouped symbols', async () => {
    ...
    const root = entrypoints.find(
      (entrypoint) => entrypoint.entrypoint === '@kjanat/dreamcli',
    );
    expect(root?.sourcePath).toBe('src/index.ts');
    expect(
      root?.kindGroups.find((group) => group.kind === 'class')?.symbols,
    ).toContainEqual({
      name: 'CLIBuilder',
      kind: 'class',
    ...
    expect(
      root?.kindGroups.find((group) => group.kind === 'function')?.symbols,
    ).toContainEqual({
      name: 'cli',
      kind: 'function',
      sourcePath: 'src/core/cli/index.ts',
    ...
    expect(
      root?.kindGroups.find((group) => group.kind === 'interface')?.symbols,
    ).toContainEqual({
      name: 'CLIOptions',
      kind: 'interface',
      sourcePath: 'src/core/cli/index.ts',
    ...
    expect(
      runtime?.kindGroups.find((group) => group.kind === 'interface')?.symbols,
    ).toContainEqual({
      name: 'RuntimeAdapter',
      kind: 'interface',
    ...
    expect(schema?.kindGroups).toEqual([
      {
        ...
        title: 'Assets',
        symbols: [
          { name: 'schema', kind: 'asset', sourcePath: 'dreamcli.schema.json' },
        ],
      },
    ]);
    ...
    expect(
      testkit?.kindGroups.find((group) => group.kind === 'function')?.symbols,
    ).toContainEqual({
      name: 'runCommand',
      kind: 'function',
