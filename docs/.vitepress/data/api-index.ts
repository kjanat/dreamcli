...
export function countPublicApiSymbols(
  entrypoints: readonly PublicApiEntrypoint[],
): number {
  return entrypoints.reduce(
    (count, entrypoint) => count + entrypoint.symbolCount,
    0,
  );
}

export function renderPublicApiIndex(
  entrypoints: readonly PublicApiEntrypoint[],
): string {
  const sections = entrypoints.flatMap((entrypoint) => {
    const caseInsensitiveCollisions =
      collectEntrypointCaseInsensitiveCollisions(entrypoint);
...
function comparePublicApiSymbols(
  left: PublicApiSymbol,
  right: PublicApiSymbol,
): number {
  const kindDelta =
...
function resolvePublicEntrypointTarget(
  rootDir: string,
  value: unknown,
): string | null {
