...
describe('typedoc normalization', () => {
  it('normalizes public entrypoints into a DreamCLI-owned docs model', {
    ...
    expect(normalized.entrypoints).toEqual([
      ...
      },
      {
        ...
        hasTypeDoc: true,
        exportIds: expect.arrayContaining([
          '@kjanat/dreamcli/runtime:RuntimeAdapter',
        ]),
        missingExports: [],
      },
      {
      ...
      },
      {
        ...
        hasTypeDoc: true,
        exportIds: expect.arrayContaining([
          '@kjanat/dreamcli/testkit:runCommand',
        ]),
        missingExports: [],
      },
    ]);
  ...
  it('preserves symbol docs, overloads, and nested property types without leaking raw TypeDoc shape', {
    ...
    const cliExport = normalized.exports.find(
      (entry) => entry.id === '@kjanat/dreamcli:cli',
    );
    expect(cliExport?.reflection.kind).toBe('function');
    expect(cliExport?.reflection.signatures).toHaveLength(2);
    expect(
      cliExport?.reflection.signatures[0]?.comment?.blockTags,
    ).toContainEqual({
      tag: '@example',
      name: null,
      content: expect.stringContaining("cli('mycli')"),
    });
    expect(cliExport?.reflection.signatures[0]?.parameters[0]?.name).toBe(
      'name',
    );
    ...
    expect(configSettings?.comment?.summary).toContain(
