...
| Resolution precedence       | Both surfaces walk one order: `CLI -> stdin -> env -> config -> prompt -> default`. An explicit `-` is CLI-sourced with bytes from stdin.                            | [CLI Semantics](/guide/semantics), [`src/core/resolve/contracts.test.ts`](https://github.com/kjanat/dreamcli/blob/master/src/core/resolve/contracts.test.ts), [`src/core/resolve/index.ts`](https://github.com/kjanat/dreamcli/blob/master/src/core/resolve/index.ts)                                                     | Resolver refactors and module splits were constrained to preserve source order and prompt/stdin gating. |
