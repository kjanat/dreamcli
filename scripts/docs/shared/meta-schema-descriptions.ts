...
const ROOT_PROPERTY_TARGETS: Readonly<Record<string, NodeTarget>> = {
	...
	defaultCommand: { exportId: 'dreamcli:CLISchema', property: 'defaultCommand' },
...
function renderDescriptionNode(node: MetaSchemaDescriptionNode, indent: number): string {
	...
	if (node.properties !== undefined) {
		...
		for (const [name, child] of Object.entries(node.properties)) {
			lines.push(`${pad}\t\t${JSON.stringify(name)}: ${renderDescriptionNode(child, indent + 2)},`);
		}
		lines.push(`${pad}\t},`);
	}
...
export function renderDefinitionMetaSchemaDescriptions(
	...
	const defs = Object.entries(descriptions.defs)
		.map(([name, node]) => `\t\t${JSON.stringify(name)}: ${renderDescriptionNode(node, 2)},`)
		.join('\n');
